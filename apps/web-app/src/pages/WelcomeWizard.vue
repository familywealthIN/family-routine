<template>
  <!-- Name capture shown BEFORE the stepper when the signed-in user doesn't
       have a usable display name. Apple sign-in only surfaces givenName /
       familyName on the first authorisation; on re-install or on users who
       tap "Hide my email", the account ends up with "Apple User" or an
       email-local placeholder. We ask for the real name here so the rest of
       the app can personalise greetings and progress reports. -->
  <div v-if="needsName && !hasProvidedName" class="name-capture">
    <div class="name-capture__inner">
      <div class="name-capture__art">
        <atom-img :src="`/img/enlightenment.png`" alt="Welcome" />
      </div>
      <h2 class="text-xs-center mt-3 mb-2">What should we call you?</h2>
      <p class="text-xs-center text-muted mb-4">
        This is just so we can greet you in your routines and reports. You
        can change it later in Settings.
      </p>
      <atom-form ref="nameForm" v-model="nameFormValid" @submit.prevent="submitName">
        <atom-text-field
          v-model="displayName"
          label="Your name"
  ...
  <atom-stepper v-else v-model="currentStep">
    <atom-stepper-header>
      <atom-stepper-step :complete="currentStep > 1" step="1">Sleep Schedule</atom-stepper-step>

      <atom-divider></atom-divider>

      <atom-stepper-step :complete="currentStep > 2" step="2">Work Hours</atom-stepper-step>

      <atom-divider></atom-divider>

      <atom-stepper-step :complete="currentStep > 3" step="3">Morning Routine</atom-stepper-step>

      <atom-divider></atom-divider>

      <atom-stepper-step :complete="currentStep > 4" step="4">Evening Activities</atom-stepper-step>
    ...
    <atom-stepper-items>
      ...
      <atom-stepper-content step="1">
        <div style="max-width: 500px; margin: 0 auto;">
          <h2 class="text-xs-center mt-4 mb-3">Set Your Sleep Schedule</h2>
          <div class="mb-3" style="max-width: 192px; margin: 0 auto;">
            <atom-img :src="`/img/night.png`" alt="Sleep Schedule">
              <template v-slot:placeholder>
                <atom-layout fill-height align-center justify-center ma-0>
                  <atom-progress-circular indeterminate color="grey lighten-5"></atom-progress-circular>
                </atom-layout>
              </template>
            </atom-img>
          </div>
          ...
          <atom-alert v-if="schedule.sleepTime && schedule.wakeTime" type="info" class="mt-3">
        ...
        <div style="max-width: 600px; margin: 0 auto;">
          ...
          <div class="mb-4">
            ...
            >
              ...
              >
                <atom-icon left>{{ activity.icon }}</atom-icon>
                {{ activity.name }}
              </atom-chip>
            </atom-chip-group>
          </div>
          ...
          <div v-if="selectedMorningActivities.length > 0" class="mb-4">
              ...
              <tbody>
                ...
                >
                  <td class="text-xs-left pa-2">
                    <atom-icon class="mr-2">{{ activity.icon }}</atom-icon>
                    {{ activity.name }}
                  </td>
            ...
            <atom-alert :type="isMorningRoutineOvertime() ? 'warning' : 'success'" class="mt-3">
      ...
      <!-- Step 2: Work Hours -->
      <atom-stepper-content step="2">
        <div style="max-width: 500px; margin: 0 auto;">
          <h2 class="text-xs-center mt-4 mb-3">Set Your Work Hours</h2>
          <div class="mb-3" style="max-width: 192px; margin: 0 auto;">
            <atom-img :src="`/img/enlightenment.png`" alt="Work">
              <template v-slot:placeholder>
                <atom-layout fill-height align-center justify-center ma-0>
                  <atom-progress-circular indeterminate color="grey lighten-5"></atom-progress-circular>
                </atom-layout>
          ...
          <atom-alert v-if="schedule.workStart && schedule.workEnd" type="info" class="mt-3">
      ...
      <atom-stepper-content step="4">
        <div style="max-width: 600px; margin: 0 auto;">
          <h2 class="text-xs-center mt-4 mb-3">Plan Your Evening Activities</h2>
          <div class="mb-3" style="max-width: 192px; margin: 0 auto;">
            <atom-img :src="`/img/relax.jpg`" alt="Evening">
              <template v-slot:placeholder>
                <atom-layout fill-height align-center justify-center ma-0>
                  <atom-progress-circular indeterminate color="grey lighten-5"></atom-progress-circular>
                </atom-layout>
          ...
          <div v-if="selectedEveningActivities.length > 0" class="mb-4">
            ...
            <atom-alert :type="isEveningRoutineOvertime() ? 'warning' : 'success'" class="mt-3">
      ...
      <atom-stepper-content step="5">
        <div style="max-width: 600px; margin: 0 auto;">
          ...
          <div class="mb-3" style="max-width: 192px; margin: 0 auto;">
            <atom-img :src="`/img/enlightenment.png`" alt="Complete">
              <template v-slot:placeholder>
                <atom-layout fill-height align-center justify-center ma-0>
                  <atom-progress-circular indeterminate color="grey lighten-5"></atom-progress-circular>
                </atom-layout>
          ...
          <atom-card class="mb-4">
            <atom-card-title>
              <atom-icon class="mr-2">schedule</atom-icon>
              Your Daily Schedule Summary
            </atom-card-title>
            <atom-card-text>
              <!-- Clock Visual Row -->
              <atom-layout row wrap class="mb-4">
                <atom-flex xs12 sm6 class="clock-section">
                  <h4 class="mb-3 clock-title">
                    <atom-icon class="mr-2" small>hotel</atom-icon>
                    Sleep Schedule
                  </h4>
                  <div class="clock-container">
                    ...
                    <div class="clock-details mt-2">
                      <p class="mb-1">
                        <atom-icon class="mr-1" small color="primary">bedtime</atom-icon>
                        Sleep: {{ schedule.sleepTime }}
                      </p>
                      <p class="mb-1">
                        <atom-icon class="mr-1" small color="success">wb_sunny</atom-icon>
                        Wake: {{ schedule.wakeTime }}
                      </p>
                      <p class="mb-0">
                        <atom-icon class="mr-1" small>timer</atom-icon>
                ...
                <atom-flex xs12 sm6 class="clock-section">
                  <h4 class="mb-3 clock-title">
                    <atom-icon class="mr-2" small>work</atom-icon>
                    Work Hours
                  </h4>
                  <div class="clock-container">
                    <svg width="150" height="150" viewBox="0 0 150 150" class="clock-svg">
                    ...
                    <div class="clock-details mt-2">
                      <p class="mb-1">
                        <atom-icon class="mr-1" small color="success">play_arrow</atom-icon>
                        Start: {{ schedule.workStart }}
                      </p>
                      <p class="mb-1">
                        <atom-icon class="mr-1" small color="error">stop</atom-icon>
              ...
              <atom-layout row wrap>
                <atom-flex xs12 sm6 class="pl-2 pr-2">
                  <h4>
                    <atom-icon class="mr-2" small>wb_sunny</atom-icon>
                  ...
                  <table v-if="getSelectedActivities('morning').length > 0" class="borderless-table">
                    <tbody>
                      ...
                      >
                        <td class="text-xs-left pa-2">
                          <atom-icon class="mr-2" small>{{ activity.icon }}</atom-icon>
                          {{ activity.name }}
                        </td>
                        <td class="text-xs-right pa-2">
                ...
                <atom-flex xs12 sm6 class="pl-2 pr-2">
                  <h4>
                    <atom-icon class="mr-2" small>nights_stay</atom-icon>
              ...
              <atom-alert type="success" class="mt-3">
                <atom-icon class="mr-2">star</atom-icon>
        ...
        <div class="text-xs-right">
          <atom-button text @click="previousStep()">Back</atom-button>
          <atom-button color="primary" @click="completeOnboarding()" :loading="creating">
...
export default {
  ...
  data() {
    return {
      ...
      // Name capture (shown pre-stepper for Apple / placeholder-name users)
      displayName: '',
      nameFormValid: false,
      hasProvidedName: false,
      nameRules: [
        (v) => !!(v && v.trim()) || 'Please enter your name',
        (v) => (v && v.trim().length >= 2) || 'Name must be at least 2 characters',
        (v) => (v && v.trim().length <= 60) || 'Name must be 60 characters or fewer',
      ],
