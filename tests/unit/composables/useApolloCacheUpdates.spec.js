...
describe('useApolloCacheUpdates', () => {
    ...
    describe('addGoalItemToCache', () => {
        it('should add goal item to existing period goal in daily cache', () => {
            ...
            expect(writeCall.data.dailyGoals[0].goalItems).toHaveLength(1);
            expect(writeCall.data.dailyGoals[0].goalItems[0].body).toBe('Test Goal');
            expect(writeCall.data.dailyGoals[0].goalItems[0].__typename).toBe('GoalItem');
        ...
        it('should update both AGENDA and DAILY caches for non-day periods', () => {
            ...
            expect(result).toBe(true);
            // Should read from both caches using dayDate
            expect(mockApolloClient.readQuery).toHaveBeenCalledWith({
        ...
        it('should handle goal item with all optional fields', () => {
            ...
            const writeCall = mockApolloClient.writeQuery.mock.calls[0][0];
            const addedItem = writeCall.data.dailyGoals[0].goalItems[0];
    ...
    describe('addMultipleGoalItemsToCache', () => {
        ...
        it('should create new period goal when none exists', () => {
            ...
            expect(result).toBe(true);
            const writeCall = mockApolloClient.writeQuery.mock.calls[0][0];
            expect(writeCall.data.dailyGoals).toHaveLength(1);
            expect(writeCall.data.dailyGoals[0].period).toBe('day');
            expect(writeCall.data.dailyGoals[0].goalItems).toHaveLength(1);
        });
        ...
        it('should handle empty goal items array', () => {
            ...
            expect(result).toBe(true);
            const writeCall = mockApolloClient.writeQuery.mock.calls[0][0];
            expect(writeCall.data.dailyGoals[0].goalItems).toHaveLength(0);
        });
    ...
    describe('updateGoalItemCompletionInCache', () => {
        it('should update goal completion status in cache', () => {
            ...
            expect(result).toBe(true);
            const writeCall = mockApolloClient.writeQuery.mock.calls[0][0];
            const updatedItem = writeCall.data.dailyGoals[0].goalItems[0];
    ...
    describe('updateSubTaskCompletionInCache', () => {
        it('should update subtask and recalculate parent progress', () => {
            ...
            expect(result).toBe(true);
            const writeCall = mockApolloClient.writeQuery.mock.calls[0][0];
            const updatedGoal = writeCall.data.dailyGoals[0].goalItems[0];
    ...
    describe('deleteGoalItemFromCache', () => {
        it('should remove goal item from cache', () => {
            ...
            expect(result).toBe(true);
            const writeCall = mockApolloClient.writeQuery.mock.calls[0][0];
            expect(writeCall.data.dailyGoals[0].goalItems).toHaveLength(2);
            expect(writeCall.data.dailyGoals[0].goalItems[0].id).toBe('goal-1');
            expect(writeCall.data.dailyGoals[0].goalItems[1].id).toBe('goal-3');
            expect(consoleLogSpy).toHaveBeenCalledWith(
    ...
    // ========================================================================
    // dual-cache updates for non-day periods
    // ========================================================================
    describe('dual-cache updates for non-day periods', () => {
        it('deleteGoalItemFromCache should update both caches for week period', () => {
            const dayDate = '23-02-2026';
            const periodDate = '28-02-2026';
            ...
            // Verify AGENDA cache was updated
            ...
            // Verify DAILY cache was also updated
            const dailyWrite = mockApolloClient.writeQuery.mock.calls[1][0];
            expect(dailyWrite.query).toBe(DAILY_GOALS_QUERY);
            expect(dailyWrite.data.dailyGoals[0].goalItems).toHaveLength(0);
        });

        it('updateGoalItemCompletionInCache should update both caches for month period', () => {
            const dayDate = '23-02-2026';
            const periodDate = '28-02-2026';

            const makeGoalData = (goalsKey) => ({
                [goalsKey]: [{
                    __typename: 'Goal',
                    id: 'goal-group',
                    date: periodDate,
            ...
            mockCacheData.set(AGENDA_GOALS_QUERY, makeGoalData('agendaGoals'));
            mockCacheData.set(DAILY_GOALS_QUERY, makeGoalData('dailyGoals'));
        ...
        it('updateSubTaskCompletionInCache should update both caches for week period', () => {
            const dayDate = '23-02-2026';
            const periodDate = '28-02-2026';
    ...
    describe('dayDate cache key resolution', () => {
        it('should use dayDate for queries when period is week', () => {
            const dayDate = '23-02-2026';
            const periodDate = '28-02-2026';
            ...
            // Should use dayDate for both queries (non-day useDayDate=true)
            expect(mockApolloClient.readQuery).toHaveBeenCalledWith({
                query: AGENDA_GOALS_QUERY,
                variables: { date: dayDate },
            });
            ...
            // writeQuery should also use dayDate
            const agendaWrite = mockApolloClient.writeQuery.mock.calls[0][0];
            expect(agendaWrite.variables).toEqual({ date: dayDate });
            const dailyWrite = mockApolloClient.writeQuery.mock.calls[1][0];
            expect(dailyWrite.variables).toEqual({ date: dayDate });
        });
    ...
    describe('cacheExists', () => {
        it('should return true when cache exists', () => {
            mockCacheData.set(DAILY_GOALS_QUERY, { dailyGoals: [] });
    ...
    describe('readCacheSafely', () => {
        ...
        it('should return custom fallback when provided', () => {
            mockCacheData.set(DAILY_GOALS_QUERY, null);
            const customFallback = { dailyGoals: [] };
