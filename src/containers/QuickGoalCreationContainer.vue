...
<script>
import QuickGoalCreation from '../components/organisms/QuickGoalCreation/QuickGoalCreation.vue';
...
export default {
  ...
  methods: {
    ...
    /**
     * Fetch related goals by goalRef using shared composable
     */
    async fetchRelatedGoals() {
      if (!this.currentGoalRef) return;

      try {
        const data = await this.$goals.fetchGoalsByGoalRef(this.currentGoalRef, {
          useCache: true,
        });
        this.relatedGoalsData = data || [];
      } catch (error) {
        console.error('Error fetching related goals:', error);
        this.relatedGoalsData = [];
      }
    },
    getGoal(period, date) {
