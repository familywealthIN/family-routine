...
export default {
  ...
  watch: {
    ...
    // Watch for mode changes from task to goal
    isTaskMode(newIsTaskMode, oldIsTaskMode) {
      // When switching from task mode to goal mode
      if (oldIsTaskMode === true && newIsTaskMode === false) {
        // If period is not day, fetch goals for that period
